...
def provision_fixtures(fixture_dir: Path, work_dir: Path) -> int:
    ...
    Python's executing verifiers run the whole corpus in one shared sandbox, so every fixture is
    visible to every row. The lane used to copy only the fixtures an utterance *named*, which is
    stricter — and the asymmetry was scored against the runtime rather than against the harness:
    `ffmpeg_084` plans `clip.mp4`, a real fixture, while the row declares `clip_4k.mp4`, so native
    was handed one file, failed on a missing input, and Python "passed" the same plan only because
    it could see the whole directory.

    **Output isolation is untouched and is the property actually worth having**: each utterance
    still gets its own directory, so a file produced by one row can never satisfy another. Inputs
    are read-only and regenerated by `just eval-fixtures`; sharing them costs nothing and removes
    a difference that was never between the runtimes.

    Hard-linked rather than copied — 847 utterances x 8.7 MB would be ~7 GB of duplication — with
    a copy fallback for filesystems or volumes that refuse the link.
    """
    linked = 0
