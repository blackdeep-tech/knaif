...
| 2026-07-27 | [portable-builds](2026-07-27-portable-builds.md) | Planning | **Blocks the first native release.** Artifacts currently inherit a runtime floor from whatever machine built them, and **both OSes have a verified P0**: every Windows binary imports `VCRUNTIME140.dll`/`MSVCP140.dll` and **none are staged**, so a clean box fails at process start; the Linux build on Ubuntu 24.04 pins **glibc 2.39**, excluding Ubuntu 22.04 LTS, Debian 12 and RHEL 9. Both share one blind spot — **`smoke.sh` runs on the build box, where neither defect can occur** — which is also why the AppImage still ships without `NOTICE` (§4(d)) months after `package.sh` was fixed. Adds a pinned `ubuntu:22.04` release container (glibc 2.35; LunarG `shaderc` for the unpackaged `glslc`), clean-room verification in Windows Sandbox and a floor-glibc container, and corrects RELEASE.md's dependency list. **Container is a release tool, not a development tool** — native builds stay first-class and contributors never touch Docker; chosen over a WSL 22.04 distro because a Dockerfile is the portable option and WSL is the Windows-only one. |
| 2026-07-17 | [post-v1-ci-and-cuda-opt-in](2026-07-17-post-v1-ci-and-cuda-opt-in.md) | Planning | The two things v1 consciously skipped: CI + `release.yml` + eval-parity (was Workstream F) and the CUDA opt-in surface (`backend install cuda` + R580 driver gate; was C6). Payload already built and proven — only the install surface is missing. Starts **after** v1.0.0 publishes; CI needs the final org. |
