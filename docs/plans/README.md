...
| 2026-07-25 | [windows-installer-polish](2026-07-25-windows-installer-polish.md) | Planning | Eleven findings — six from a live v1.0.1 install session, five from a follow-up script cross-read. **P0s:** undeclared `deps` parent task defeats three `unchecked` flags (AGPL Ghostscript + LibreOffice pre-checked); a missing `_is1` key leaves no Add/Remove entry and no upgrade detection; and **`NOTICE` is never distributed** (F11 — Apache-2.0 §4(d)). Plus `getmodel` offered when the GGUF exists, nothing saying what knaif is or who maintains it (F8), no icon/VERSIONINFO, no `[InstallDelete]` so deselected components survive, and an `.iss` lint to stop F2 recurring. **F8 + F11 are `package.sh`, so they hit all three OS artifacts.** Owner decisions 2026-07-25: copyright holder is Blackdeep Technologies (fix `LICENSE:189`), knaif stays OSS ⇒ **SignPath Foundation**, no CLA ever needed. License-accept default already fixed. |
