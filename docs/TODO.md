...
- [ ] **RE-LOCK BOTH SNAPSHOTS — the only thing blocking the 2026-09-17 audit fixes.** `POLICY_VERSION` is now **3**: the executing runner grants confirmation during command capture, which moves `avg_knaif_score`'s denominator exactly as the 1→2 bump did. Measured on a fixed model with identical predictions on all 1,015 utterances, ffmpeg moves `0.937720 → 0.938895` outcome and `0.977945 → 0.980123` knaif; documents does not move. Both `eval_snapshot.json` files are still policy-2 records, so **four tests fail by design** until they are re-locked — each says so in its own message:
  - `test_acceptance.py::test_the_accepted_baseline_clears_its_own_floors[ffmpeg|documents]`
  - `test_acceptance_cmd.py::test_accept_passes_on_the_accepted_baseline`
  - `test_native_acceptance.py::test_the_command_accepts_a_run_that_clears_the_real_bar`

  **The evidence is already measured** — [`evals/runs/2026-09-17_policy-v3-integration_success`](../evals/runs/2026-09-17_policy-v3-integration_success/report.md), same model bytes and argv as the audit control, on the integrated tree. ffmpeg reproduces the corrected instrument to ten decimal places (`0.9388954172` / `0.9801227169`, 851 utterances, coverage 1.0, safety 11/11) and `ffmpeg_282` quarter-speed executes and scores 1.0. documents outcome reproduces exactly; its artifact average came out `0.9978354978` rather than `1.0` (see the determinism item below). Both skills clear the hardened gate on it — 39 and 36 thresholds — so the re-lock is expected to pass. Do it in its own commit, per AGENTS.md.
- [ ] **The eval lane is not bitwise reproducible, and nothing says so** — inference runs at `temperature=0.0`, yet `documents_042#0` emitted `order: "-1"` on one run and `order: "reverse"` on two others from identical model bytes and prompt (llama.cpp GPU reduction order, not sampling). One row in 1,015, but a single documents row is **0.22 pp of `avg_knaif_score`** on a 154-row denominator. Consequence: artifact-average deltas below roughly half a point are not decision-grade on documents. The 2026-09-17 candidate rejection is unaffected — it rests on three *outcome* regressions, and outcome accuracy reproduced exactly on both skills — but its artifact component (1.0 → 0.99669) is within this noise and should not be quoted alone. Measure the actual run-to-run spread before any future decision leans on a small artifact delta.
