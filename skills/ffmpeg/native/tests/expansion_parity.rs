...
/// even though the tests share a process. The directory is the contract's own `sandbox` (committed,
/// so CI needs no generated fixtures), read from the fixture file like the documents consumer.
fn enter_fixture_dir() -> PathBuf {
    let doc_sandbox = repo_root().join(fixtures()["sandbox"].as_str().expect("sandbox key"));
